"""
WebSocket Server for Real-Time rPPG Inference with PhysNet
Faster than REST API - uses persistent connection and binary frames
PhysNet expects input shape: [batch, 3, frames, height, width]
"""
...
config = None
frame_buffer = []
chunk_size = 128  # PhysNet expects 128 frames


def calculate_bpm_from_ppg(ppg_signal, fs=35, low_pass=0.6, high_pass=3.3):
    """
    Calculate heart rate in BPM from PPG signal using FFT.
    
    Args:
        ppg_signal: PPG signal array (1D or 2D)
        fs: Sampling rate (frames per second), default=35 for PhysNet
...
def preprocess_for_bpm(ppg_signal, fs=35):
    """
...
def calculate_respiratory_rate_from_ppg(ppg_signal, fs=35, low_pass=0.13, high_pass=0.5):
    """
    Calculate respiratory rate in breaths per minute from PPG signal using FFT.
    
    Args:
        ppg_signal: PPG signal array (1D or 2D)
        fs: Sampling rate (frames per second), default=35
...
def calculate_hrv_from_ppg(ppg_signal, fs=35):
    """
...
def preprocess_for_rr(ppg_signal, fs=35):
    """
...
def initialize_model(config_path):
    """Initialize the PhysNet model for inference"""
    ...
    # Initialize model - PhysNet expects [batch, 3, frames, height, width]
    frame_num = config.MODEL.PHYSNET.FRAME_NUM  # Default 128
    model = PhysNet_padding_Encoder_Decoder_MAX(frames=frame_num).to(device)
    ...
    print(f"✓ Model loaded successfully (frames={frame_num})")
...
def compute_diff_normalized(frames):
    """Compute diff-normalized frames for PhysNet"""
...
def predict_from_frames(frames, config):
    ...
    # Compute diff-normalized for PhysNet
    diff_frames = compute_diff_normalized(raw_frames)
    
    # Convert to PhysNet format: [batch, channels, time, height, width]
    # PhysNet expects [B, 3, T, H, W] where T=128 frames
    tensor = torch.from_numpy(diff_frames).float()  # (num_frames, H, W, 3)
    tensor = tensor.permute(3, 0, 1, 2)  # (3, num_frames, H, W)
    tensor = tensor.unsqueeze(0)  # (1, 3, num_frames, H, W)
    ...
    # Convert to numpy - rPPG shape: [batch, length]
    predictions_np = rPPG[0].cpu().numpy()  # Get first (and only) batch
    
    # Normalize predictions
    predictions_np = (predictions_np - np.mean(predictions_np)) / (np.std(predictions_np) + 1e-7)
    
    # Calculate BPM and Respiratory Rate from the prediction
    fs = 35  # Video sampling rate for PhysNet (frames per second)
                        ...
                        if len(frame_buffer) >= chunk_size:
                            try:
                                ...
                                response = {
                                    ...
                                    'sl': stress.get('stress_level', 'normal'),
                                    'ab': stress.get('autonomic_balance', 'balanced')[0],
                                    'wi': workload.get('workload_index', 0),
                                    'wl': workload.get('workload_level', 'moderate'),
...
async def main():
    ...
    if len(sys.argv) > 1:
        ...
        config_path = 'configs/infer_configs/PURE_UBFC-PHYS_PHYSNET_BASIC.yaml'
    
    print("\n" + "=" * 60)
    print("Initializing rPPG WebSocket Server (PhysNet)")
    print("=" * 60)
